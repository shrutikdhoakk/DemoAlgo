# backtest_quick.py — portfolio backtester aligned with SwingStrategy rules + optional chart-pattern filters
...
# Pattern filters (optional; ON by default; fails-open if helper modules not present)
ENABLE_PATTERN_FILTERS = os.getenv("ENABLE_PATTERN_FILTERS", "1") == "1"
PAT_MIN_BONUS   = int(os.getenv("PAT_MIN_BONUS", "2"))   # require >=2 pattern “hits”
...
def backtest(symbols: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # pattern gate wrapper (lazy import; fail-open)
    def _pattern_gate(df_: pd.DataFrame) -> dict:
        if not ENABLE_PATTERN_FILTERS:
            return {"ok": True, "bonus": 0, "flags": {}, "why": "disabled"}
        try:
            try:
                from .pattern_filters import pattern_gate
            except Exception:
                from backtesting.pattern_filters import pattern_gate
            # ensure knobs present
            ...
            return pattern_gate(df_)
        except Exception as e:
            print(f"[bt] pattern_filters unavailable: {e}. Continuing without gate.")
            return {"ok": True, "bonus": 0, "flags": {}, "why": "missing_module"}
            ...
            if remaining_budget > 0 and cash > 0:
                ...
                for sym, df in data_map.items():
                    ...
                    if v < VOL_MULT * vavg:
                        continue
                    # breakout conditions
                    tol = 1.0  # align with live: set 0.995 if you want slight tolerance
                    ...
                    # --- Chart pattern gate / bonus (optional) ---
                    # Use up to ~60 bars context (or larger than triangle window)
                    ctx = df.loc[:dt].tail(max(60, PAT_TRI_WINDOW + 5))
                    pg = _pattern_gate(ctx)
                    if not pg.get("ok", True):
                        # print(f"[{sym}] rejected by patterns: {pg.get('why')} flags={pg.get('flags')}")
                        continue
                    ...
                    # pattern bonus (0..3) * 0.5
                    bonus = float(pg.get("bonus", 0.0))
                    score += 0.5 * bonus
